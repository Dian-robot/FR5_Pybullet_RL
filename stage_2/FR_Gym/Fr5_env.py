...
class FR5_Env(gym.Env):
    ...
    def init_env(self):
        ...
        self.fr5 = self.p.loadURDF(
            r"D:\postgraduate\project\FR_Reinforcement\stage_2\fr5_description\urdf\fr5v6.urdf",
            # "/root/FR5_noise/fr5_description/urdf/fr5v6.urdf",
        ...
        collisionTargetId = self.p.createCollisionShape(shapeType=p.GEOM_CYLINDER,
                                                        radius=0.04, height=self.table_height)
        self.targettable = self.p.createMultiBody(baseMass=0,  # 质量
                                                  baseCollisionShapeIndex=collisionTargetId,
                                                  basePosition=[0.5, 0.5, 2])
        # 创建初始底座
        self.base_height = 0.04
        baseId = self.p.createCollisionShape(shapeType=p.GEOM_CYLINDER,
                                             radius=0.04, height=self.base_height)
        self.base = self.p.createMultiBody(baseMass=0,  # 质量
                                           baseCollisionShapeIndex=baseId,
                                           basePosition=[0.5, 0.5, 2])
    ...
    def reset(self, seed=None, options=None):
        ...
        self.p.resetBasePositionAndOrientation(self.base, self.base_position, [0, 0, 0, 1])
        # 重新设置机械臂的位置
        ...
        # 重新设置夹爪位置
        # 随机生成初始位置
        init_x = np.random.uniform(0.1, 0.5, 1)[0]
        # init_x = 0.3
        init_y = np.random.uniform(0.1, 0.35, 1)[0]
        # init_y = 0.5
        init_z = np.random.uniform(0.07, 0.08, 1)[0]
        # init_z = 0.08
        euler_x = np.random.uniform(-0.05, 0.15, 1)
        euler_y = np.random.uniform(-0.15, 0.15, 1)
        euler_z = np.random.uniform(-0.2, 0.2, 1)
        # init_z = np.random.uniform(0.03 + self.cup_height / 2, 0.07 + self.cup_height / 2, 1)
        ...
        self.goalx = np.random.uniform(-0.1, 0.1, 1)[0]
        # self.goalx = 0.07
        self.goaly = np.random.uniform(0.6, 0.9, 1)[0]
        # self.goaly = 0.75
        # self.goalz = np.random.uniform(self.table_height + self.cup_height / 2 - 0.01,
        #                                self.table_height + self.cup_height / 2 + 0.05, 1)[0]
        self.goalz = np.random.uniform(0.1, 0.12, 1)[0]
        # self.goalz = 0.12
        self.p.resetBasePositionAndOrientation(self.targettable,
                                               [self.goalx, self.goaly,
                                                self.goalz - self.cup_height / 2 - self.table_height / 2],
                                               [0, 0, 0, 1])
        ...
        self.base_position = [gripper_centre_pos[0], gripper_centre_pos[1],
                              # self.base_height / 2]
                              gripper_centre_pos[2] - self.cup_height / 2 - self.base_height / 2]
        self.p.resetBasePositionAndOrientation(self.base, self.base_position, [0, 0, 0, 1])
        self.target_position = [gripper_centre_pos[0], gripper_centre_pos[1],
                                gripper_centre_pos[2] - self.cup_height / 2]
    ...
    def get_observation(self, add_noise=True, SPF = True):
        ...
        for i in [1, 2, 3, 4, 5, 6]:
            ...
            if add_noise == True:
                joint_angles[i - 1] = self.add_noise(joint_angles[i - 1], range=0)
...
if __name__ == "__main__":
    ...
    Env.reset()
    # x 轴
    frame_posture = [0, 0, 0, 1]
    # 固定夹爪相对于机械臂末端的相对位置转换
    frame_start_postition = Env.get_gripper_position()
