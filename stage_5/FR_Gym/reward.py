...
def cal_success_reward(self, distance):
    ...
    target_obs_contact_points = p.getContactPoints(bodyA=self.target, bodyB=self.obstacle)
    ...
    if target_table_contact_points:
        table_contact = True

    # 碰撞障碍物
    if self_obstacle_contact_points or target_obs_contact_points:
    ...
    #     success_reward = - 1
    #     self.terminated = True
    #     logger.info("失败！未抓取到目标！ 执行步数：%s    距离目标:%s" % (self.step_num, distance))
...
def grasp_reward(self):
    ...
    judge_success(self, distance, pose_reward, success_dis=0.02, success_pose = -7)
