...
class FR5_Env(gym.Env):
    ...
    def init_env(self):
        ...
        self.fr5 = self.p.loadURDF(
            r"D:\postgraduate\project\FR_Reinforcement\stage_4\fr5_description\urdf\fr5v6.urdf",
    ...
    def reset(self, seed=None, options=None):
        ...
        init_z = self.button_height + np.random.uniform(-0.01, 0.06, 1)[0]
        # init_x = 0.117
        # init_y = 0.555
        # init_z = 0.3
        ...
        for i in range(3):
            joint_angles = p.calculateInverseKinematics(self.fr5, 6, pos, orn,
                                                        ...
                                                        jointRanges=jr,
                                                        restPoses=rp)
            joint_angles = list(joint_angles[:6]) + self.grasp_effort
            p.setJointMotorControlArray(self.fr5, [1, 2, 3, 4, 5, 6, 8, 9], p.POSITION_CONTROL,
        ...
        self.goaly = gripper_centre_pos[1] + np.random.uniform(-0.03, 0.03, 1)[0]
        self.goalz = self.cup_height / 2 + self.table_height + np.random.uniform(-0.02, 0.06, 1)[0]
        # self.goalx = 0.07
        # self.goaly = 0.75
        # self.goalz = 0.12
        # self.goalz = self.cup_height / 2 + self.table_height + 0.06
        # safe_dis = 0.02  #障碍物高于夹爪的距离
        self.obstacle_position = [self.goalx,
                                  gripper_centre_pos[1] + np.random.uniform(0.01, 0.03, 1)[0],
                                  self.button_height]
        self.p.resetBasePositionAndOrientation(self.obstacle, self.obstacle_position, [0, 0, 0, 1])
        Euler = [math.pi / 2, 0, 0]
        ...
        self.button_position = [gripper_centre_pos[0], gripper_centre_pos[1] + self.button_length / 2 + 0.02,
                                init_z]
        self.p.resetBasePositionAndOrientation(self.button, self.button_position, key_orn)

        self.targettable_position = [self.goalx, self.goaly, self.goalz - self.cup_height / 2 - self.table_height/2]
        ...
        # 第一阶段后撤观察杯子
        self.goalx = self.goalx + np.random.uniform(-0.02, 0.02, 1)[0]
        # self.goalx = self.goalx + 0.02
        self.goaly = self.goaly - self.obstacle_wide - 0.12 + np.random.uniform(-0.1, 0.02, 1)[0]
        # self.goaly = self.goaly - self.obstacle_wide - 0.07
        self.goalz = self.goalz + self.cup_height + np.random.uniform(-0.04, 0.04, 1)[0]
        # self.goalx = 0.07
        # self.goaly = 0.6
        # self.goalz = 0.16
    ...
    def add_debug_line(self):
        ...
        # x 轴
        # 固定夹爪相对于机械臂末端的相对位置转换
        frame_start_postition, frame_posture = p.getBasePositionAndOrientation(self.button)
